...
| TC-246 | Flota y expediente (`buildFleetTimeline`, `buildAgvDossier`) | Un tramo de cobertura corto sin lecturas del AGV no es «leyendo»; un AGV cuya última lectura cae en una exportación anterior no tiene silencio abierto | Ritmo normal dibujado sin lecturas; «sin leer desde» medido contra un hueco entre exportaciones | Unitaria |
| TC-247 | Calles de carga (`readCoLanes`, `readLapAnchors`) | Tag repetido, mismo `orden`, mismo tag en dos papeles y parada o salida compartida no se montan y lo dicen; entrada común avisa; `orden` a medias manda el fichero | Una calle mal declarada montada en silencio; un `orden` vacío ordenado «primero» | Unitaria |
| TC-248 | Alineación de anillos (`reconcileCircuitOrder`, `compareAgainstVsystem`, `stableAnchors`) | Con el primer tag de la lista mal colocado, solo él sale en otro sitio; con un tag movido, las anclas sanas que le siguen se conservan | Media vuelta de tags sanos acusados por rotar al primer tag | Unitaria |
| TC-249 | Suma entre anclas (`anchor-sums`) | Ventanas abiertas en el corte: un tag nuevo leído en el mismo instante que el ancla siguiente sale insertado dentro del fichero; dos pasadas de un solo AGV sin un tag sano quedan sin afirmar, con dos AGV se afirma | Un insertado que no se enseña a resolución de minuto; «retirado» por las omisiones de un solo lector | Unitaria |
| TC-250 | Cambios de tag y tags fuera de la lista (`tag-changes`, `undeclared-tags`, `drift`, `list-cleanup`) | Con una pasada menos que el mínimo sin leerlo no «deja de leerse» y con el mínimo justo sí; el declarado sin lecturas del final de la lista se nombra cuando el sitio da la vuelta; una lectura de día fuera de su sitio no convierte un tag de noche en candidato, y una en su sitio sí; `desaparecido` con una lectura lleva soporte débil; un refuerzo sin vecino leído se juzga con el anillo | Umbral cumplido con una pasada de menos; tag de noche convertido en tag real por una lectura suelta; refuerzo «separado» sin razón | Unitaria |
| TC-251 | Ingesta (`importer`, `catalog`, `fleet-history`, `union`, `monotonicity`) | Comillas envolventes fuera del identificador y `""` deshecha; cabecera con BOM, comillas y acentos reconocida y columnas desconocidas avisadas; tres cortes solapados conservan las tres procedencias; los pares con hora repetida no cuentan como inversión y una pila íntegra que cruza el 25/10 no tiene inversiones; las líneas en blanco se cuentan aparte y `totalRows` cuadra | Un tag con comillas como nodo distinto; procedencia pisada; inversión falsa por la hora repetida | Unitaria |
| TC-252 | Cobertura y orden (`graph`, `read-matrix`, `laps`, `semantic-hash`, `cohort`) | La cola cortada de una exportación no se empareja con la primera lectura de la siguiente ni cierra vuelta; una lectura en hora repetida no forma transición ni pasada; Map/Set/Date se rechazan en el hash; los empates de cohortes se ordenan por código | Una arista o una vuelta de 30 días; dos estados distintos con el mismo hash | Unitaria |
| TC-253 | Inventario (`buildTagInventory`) | Un tag que solo está en la lista `critico` sale `critico-no-declarado` con la acción de comprobar la lista | «Declarado sin memoria» que induce a cargar una errata en la memoria | Unitaria |
| TC-254 | Estado del circuito (`concentrated`, `declaredWithoutReadings`, `vehiclePace`) | Un sitio o un AGV sin exposición no se señala; con los extremos de la zona escritos en otro orden en la lista, la zona no nombra un tag sin lecturas de otro sitio; cada AGV se compara con la mediana de los demás y con menos de tres probados no hay veredicto | Concentración con cero pasadas; causa de otro punto del circuito; diferencia repartida entre dos AGV | Unitaria |
| TC-255 | Cobertura en carga y FIFO (`buildChargingReport`, `buildFifoReport`) | Una estancia o una pasada que cruza el hueco entre dos exportaciones es incompleta: sin permanencia larga ni adelantamiento | Permanencia de 24 h y «adelantado por todos» por un hueco sin datos | Unitaria |
| TC-256 | Flujo (`flowStops`, `segment-bands`, `silence-kind`) | Doce minutos releyendo un tag es una parada sin explicación medida contra el tramo que sale de él; un AGV que calla horas no retiene si otros atravesaron su último tag; sin lo habitual del tramo no se afirma «parado»; reaparecer por detrás no se mide; quien entró a cargar durante la parada de la producción cuenta y siguió por su sitio | Parada invisible; retenedor fantasma; «parado» sin referencia; media vuelta hacia atrás leída como casi una vuelta hacia delante | Unitaria |
| TC-257 | Puntos críticos (`classifyCrossings`) | Una rama que está en el camino de la otra se propone como cruce con la duda dicha (`maybeSkipped`); dos ramas de verdad que reconvergen siguen siendo cruce sin duda | Un cruce afirmado donde solo hay un tag que no siempre se lee | Unitaria |
| TC-258 | Alimentación de la línea (`measureLineFeed`) | Con tres descansos de 20 min la zona del pulmón, sus miembros y su capacidad son los mismos que sin descansos; solo con descansos, el pulmón queda sin medir; un tiempo entre pasos que cruza el cambio de régimen va al régimen de su punto medio | Toda la flota como pulmón; una parada de la línea callada por cruzar las 22:00 | Unitaria |
...
| TC-266 | Hora repetida por posición (`resolveRepeatedHourByPosition`, importador) | Pila íntegra con las dos ocurrencias, en los dos sentidos: todas resueltas, cero ambiguas, cero inversiones, aristas correctas; racha sin retroceso seguida de 03:xx: segunda; precedida por 01:xx sin 03:xx: sigue ambigua; sin sentido medible no se resuelve; varios retrocesos son desorden; marzo no cambia; una resuelta sí forma transición y cuenta como pasada | Afirmar una ocurrencia que el fichero no sostiene; un instante inventado en marzo | Unitaria |
| TC-267 | Secciones entre anclas (`anchor-sections`) | Anillo de 12 con anclas en 0, 4 y 8 y dos AGV: tres secciones con su p50; el p50 por fichero solo con los pasos de esa ventana y el mínimo de muestras; se descartan el paso que cruza un hueco de cobertura, el que cruza una parada de la producción y el que tiene una calle de carga en medio; relecturas seguidas colapsadas; saltarse un ancla descarta el paso; nombre por mayoría estricta de la lista `tramo`; una sola ancla, sin secciones | Una sección medida con una carga dentro; un nombre elegido a partes iguales | Unitaria |
| TC-268 | Vueltas por ancla (`segmentLaps`) | Dos lecturas seguidas del ancla no fabrican una vuelta de segundos; un AGV con una sola lectura sale como vuelta desconocida; las relecturas finales no abren una parcial | Vueltas de dos segundos; un AGV que desaparece del recuento | Unitaria |
| TC-269 | Ritmo de la línea (`measureLineFeed`, `aboveFenceMs`) | Con el ciclo entre 50 y 60 s y ninguna parada, «por encima del ciclo local» suma algo y «sin paso» es 0; con una parada de 5 min, «sin paso» suma su exceso sobre la valla | Paradas inventadas por la variación normal del ciclo | Unitaria |
| TC-270 | «Deja de leer» en producción (`abandonedReadings`) | Lecturas a 0 h y 8 h con la noche en medio y luego cada 5 min: la referencia son los 5 min de producción, la noche queda fuera y el silencio final se declara con su cifra; si el hueco de producción de referencia lo justifica, no se declara | Una noche vivida que tapa siete horas de silencio | Unitaria |
...
| TC-276 | Instantánea (`buildSnapshot`, `compareSnapshots`) | Construcción canónica y sellado de versión; inválidos rechazados con motivo; cada delta de vértice (aparece, desaparece, se mueve por vecino y no por índice, cambia de clase, deja de leerse con y sin prueba de azar, empieza a leerse) y de arista por régimen; vuelta que cambia | Un «se mueve» por desplazamiento de índices; un «deja de leerse» con tres pasadas | Unitaria |
| TC-277 | Evolución desde instantáneas (`historiesFromSnapshots`, `structureBetweenSnapshots`, `driftBetweenSnapshots`) | Las historias coinciden con `segmentHistories` sobre las mismas horquillas; retirado con dos AGV y sin afirmar con uno; insertado; sustituido en el mismo sitio y en otro punto, tres seguidos; sin anclas comunes, nada; deriva afirmada y sin afirmar, sustitución candidata, AGV que deja de leer y que no adopta | Un cambio de estructura afirmado por un solo AGV; una deriva sin su cifra | Unitaria |
