...
| DATA_CONTRACTS §3.7 / TC-204–206 listas e historial en Excel, y borrador del circuito | `src/persistence/xlsx.ts`, `src/domain/list-templates.ts`, `src/ingestion/catalog.ts` (`importCatalogRows`), `src/ingestion/fleet-history.ts` (`importFleetRows`, `excelSerialToText`), `workers/import.worker.ts` (lee `.xlsx`), `src/presentation/excel-ui.ts`, `src/presentation/main.ts` (botones), `src/application/protocol.ts` (`vsystemCohortId`) | `tests/unit/xlsx.test.ts`, `tests/unit/list-templates.test.ts`, `tests/e2e/vistas-diagnostico.spec.ts` |
